...
rospy.init_node('niryo_one_example_test_translation')
publisher_stream = rospy.Publisher('~video_stream', Image, queue_size=1)
rate = rospy.Rate(1)
...
try:
    while not rospy.is_shutdown():
        ...
        if (list_good_candidates!=None):
            ...
            # angle2 = math.acos((np.dot(v_ws,u_ws))/(np.linalg.norm(u_ws)*np.linalg.norm(v_ws)))
            ...
            print(angle)
            # print(angle2)
            success, new_img = m.draw_markers(img)
            for k in range (4):
                cv2.circle(new_img, (temoin[k][0], temoin[k][1]), 20, (255, 0, 0), 2)
                cv2.putText(new_img, "{}".format(k + 1),(temoin[k][0] + 5, temoin[k][1] - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 3)
                cv2.putText(new_img, "{}".format(k + 1),(temoin[k][0] + 5, temoin[k][1] - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
            filename = '/home/niryo/catkin_ws/src/niryo_one_python_api/examples/imageMarkTest.jpg'
            cv2.imwrite(filename, new_img)

            #Publisher
            msg2 = CvBridge().cv2_to_imgmsg(new_img,encoding="bgr8") 
            publisher_stream.publish(msg2)
        else:
            print("new turn")
        rate.sleep()
